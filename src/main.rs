...
async fn main() -> Result<()> {
    ...
    let url = make_url(
        api_key,
        "json".to_string(),
        "realtimeStationArrival".to_string(),
        "0".to_string(),
        "5".to_string(),
        "동천".to_string(),
    );
...
fn make_url(
    ...
    const SEOUL_URL: &str = "http://swopenAPI.seoul.go.kr/api/subway";
    let encodec_station_name = encode(&station_name);
    let full_url = format!(
        "{}/{}/{}/{}/{}/{}/{}",
        SEOUL_URL, api_key, file_type, service_name, start_index, end_index, encodec_station_name
    );
